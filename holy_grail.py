...
def holy_light_hpms(bh, crit, haste, mana, mp5, int, min_h, max_h, spellh, spelllvl):
    ...
    heal_c = heal_nc * 1.5
    heal_av = heal_nc * (1 - real_crit - int * 0.000125 * 1.1**2) + heal_c * (real_crit + int * 0.000125 * 1.1**2)
    ...
    if libram_var.get() == "truth":
        ...
        mana_av = mana_eff * (1 - real_crit - int * 0.000125 * 1.1**2) + (mana_eff - mana_lib * 0.6) * (real_crit + int * 0.000125 * 1.1**2)
    else:
        mana_eff = mana - mana_mp5
        mana_av = mana_eff * (1 - real_crit - int * 0.000125 * 1.1**2) + (mana_eff - mana * 0.6) * (real_crit + int * 0.000125 * 1.1**2)
...
def total_hpms(stats):
    ...
    if need_mana_var.get():
        return share9*x9 + share11*x11 + stats["int"]*0.0025
    else:
        return share9*x9 + share11*x11
...
intro_text = (
    ...
    "  (remember to include +5% crit from Holy Power).\n"
...
tk.Label(options_frame, text="Libram", font=("Arial",10,"bold")).grid(row=0, column=1, sticky="w", padx=15)
tk.Label(options_frame, text="Gems", font=("Arial",10,"bold")).grid(row=0, column=2, sticky="w", padx=15)
tk.Label(options_frame, text="Mana", font=("Arial",10,"bold")).grid(row=0, column=3, sticky="w", padx=15)
...
tk.Radiobutton(
    ...
    variable=libram_var,
    value="mending"
).grid(row=4, column=1, sticky="w", padx=15)
...
# ----- Mana-----
need_mana_var = tk.BooleanVar(value=False)

tk.Checkbutton(
    options_frame,
    text="Need Mana",
    variable=need_mana_var
